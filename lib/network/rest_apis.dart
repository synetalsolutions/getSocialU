...
Future<GroupModel> getGroupDetail({int? groupId, String? userId}) async {
  Map request = {
    "group_id": groupId,
    "current_user_id": userId ?? appStore.loginUserId,
  };

  return GroupModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.getGroupDetail}', method: HttpMethod.POST, request: request)));
}
...
Future<MemberDetailModel> getMemberDetail({required int userId}) async {
  ...
  return MemberDetailModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.getMemberDetail}', method: HttpMethod.POST, request: request)));
}
...
Future<List<FriendRequestModel>> getFriendList({
  ...
  FriendListResponse res = FriendListResponse.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.getFriendList}', method: HttpMethod.POST, request: request),isFriendList: true));
...
Future<List<StoryResponseModel>> getUserStories({int? userId}) async {
  Iterable it;
  StoryResponseModel userStory;

  if (userId != null) {
    Map request = {"user_id": appStore.loginUserId.toInt()};

    userStory = StoryResponseModel.fromJson(
      await handleResponse(await buildHttpResponse('${APIEndPoint.getUserStories}', method: HttpMethod.POST, request: request)),
    );
    return [userStory];
  } else {
...
Future<TopicModel> getTopicDetail({required int topicId, int page = 1}) async {
  ...
  return TopicModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoint.topicDetails}', method: HttpMethod.POST, request: request)));
}
