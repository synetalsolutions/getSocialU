...
import 'package:socialv/models/messages/start_call_model.dart';
...
Future<List<UserSettingsModel>> userSettings() async {
  Iterable it = await handleResponse(await buildHttpResponse(MessageAPIEndPoint.userSettings));

  return it.map((e) => UserSettingsModel.fromJson(e)).toList();
}
...
Future<StartCallModel> startCall({required Map request}) async {
  return StartCallModel.fromJson(await handleResponse(await buildHttpResponse(MessageAPIEndPoint.callStart, method: HttpMethod.POST, request: request)));
}

Future<dynamic> callStarted({required Map request}) async {
  return StartCallModel.fromJson(await handleResponse(await buildHttpResponse(MessageAPIEndPoint.callStarted, method: HttpMethod.POST, request: request)));
}

Future<dynamic> callUsage({required Map request}) async {
  return StartCallModel.fromJson(await handleResponse(await buildHttpResponse(MessageAPIEndPoint.callUsage, method: HttpMethod.POST, request: request)));
}

Future<dynamic> callMissed({required Map request}) async {
  return await handleResponse(await buildHttpResponse(MessageAPIEndPoint.callMissed, method: HttpMethod.POST, request: request));
}
