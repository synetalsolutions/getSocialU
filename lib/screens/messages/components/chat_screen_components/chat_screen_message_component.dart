    ...
    if (messageText.validate() != MessageText.onlyFiles) {
      if (threadType != 'group') {
        if (notBlocked || isLoggedInUser.validate()) {
          if (messageText.contains('href') || messageText.contains('<br />')) {
            return HtmlWidget(postContent: messageText.validateAndFilter(), color: isLoggedInUser.validate() ? Colors.white : context.iconColor);
          } else {
            return Text(
              parseHtmlString(messageText.validateAndFilter()),
