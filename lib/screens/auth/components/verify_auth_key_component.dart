              ...
              children: [
                ...
                AppButton(
                  ...
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      cachedImage(ic_activate, color: Colors.white, height: 20, width: 20),
                      6.width,
                      Text(language.activate, style: boldTextStyle(color: Colors.white)),
                    ],
                  ).fit(),
                  onTap: () {
                    if (verifyAuthFormKey.currentState!.validate() && authKey.text.isNotEmpty) {
                      verifyAuthFormKey.currentState!.save();
                      hideKeyboard(context);
                      widget.onSubmit?.call(authKey.text);
                      finish(context);
                    } else {
                      toast(language.pleaseEnterAuthkey);
                    }
                  },
                ).expand(),
              ],
